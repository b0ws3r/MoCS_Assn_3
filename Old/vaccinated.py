...
def graph_exponential_dist():
    z = [int(np.random.exponential(scale=100)) for i in range(1000)]
    if sum(z)%2 != 0:
        z[0] += 1
...
def run_model():
    ...
    for t in T:
        ...
        for k in range(len(degree_freq)):
            ...
            delta_Svk = (1 - rho) * -beta * k * theta * Ivk[k] + alpha * Ivk[k] - Ik[k]
            delta_Ik = beta * k * theta * Sk[k] - alpha * Ik[k]
            delta_Ivk = (1 - rho) * beta * k * theta * Svk[k] - alpha * Ivk[k] - Ik[k]
